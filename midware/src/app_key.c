...
static key_map_t keys_map[] = {
        [KEY_BEEP]    = { M_KEY_LEFT_PORT,  M_KEY_LEFT_PIN,  FSM_EVENT_PRESS_BEEP,    FSM_EVENT_RELEASE_BEEP    },
        [KEY_LOG]     = { M_KEY_MID_PORT,   M_KEY_MID_PIN,   FSM_EVENT_PRESS_LOG,     FSM_EVENT_RELEASE_LOG     },
...
static __always_inline void key_gpio_irq_handle(int i)
{
        ...
        if (GPIO_KEY_PRESS(keys_map[i].gpio_port, keys_map[i].gpio_pin)) {
                ...
                *last_pressed = g_fsm.curr->state;
                ...
                // BUG:
                // if return back to same state, the key just released,
                // release event still will input need to clear
                // to prevent input which was intended for previous state
