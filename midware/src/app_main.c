...
fsm_state_t state_main_enter(fsm_node_t *node, fsm_event_t event)
{
        ...
        case FSM_EVENT_SCAN_DONE:
                ...
                if (is_temp_valid(&g_temp_thres[scan_mode], last_result)) {
                        ...
                        scan_log_write_idx(&log_uv[scan_mode],
                                           scan_log->last_write,
                                           last_uv);
                        scan_log_write_idx(&log_ntc[scan_mode],
                                           scan_log->last_write,
                                           last_ntc);
#endif
                }
...
fsm_state_t state_main_proc(fsm_node_t *node, fsm_event_t *out)
{
        ...
        if (log_show_uv) {
                ...
                lcd_number_show(LCD_LOGNUM, LCD_ALIGN_LEFT,
                                log_number, 0, LCD_NO_DOT);
                lcd_number_show(LCD_IDXNUM, LCD_ALIGN_LEFT,
                                ntc_number, 0, LCD_NO_DOT);
        }
#endif
...
lcd_update:
        ...
        if (g_rt->scan_done) {
                if (g_cfg->beep_on) {
                        if (scan_mode == SCAN_BODY &&
                            big_number >= g_cfg->body_alarm_C) {
                                delay_budget -= body_beep_alarm();
                        } else {
                                // user may wanna release trigger
                                // after hearing beep so put a little delay here
